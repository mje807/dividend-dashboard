...
import { Search, ArrowUpDown, ChevronUp, ChevronDown, ExternalLink } from "lucide-react";
import { royaltyLastUpdated, type RoyaltyStock } from "~/data/royalty";
import { type RoyaltyMetrics } from "~/data/royalty-metrics";
import { type StockAnalysis } from "~/data/royalty-analysis";
import { calcAttractiveness } from "~/utils/attractiveness";
import { getRoyaltyAnalysesLatest, getRoyaltyMetricsLatest, getRoyaltyUniverseLatest } from "~/lib/market-data.server";
...
type WatchlistLoaderData = {
  universe: RoyaltyStock[];
...
function normalizeYieldPercent(raw: number | null | undefined): number {
  ...
  if (raw >= 30 && raw <= 1000) return raw / 100;
  return raw;
}
...
export default function Watchlist() {
  const { universe, metrics, analyses } = useLoaderData<WatchlistLoaderData>();
  const metricsMap = useMemo(() => new Map(metrics.map((m) => [m.ticker, m])), [metrics]);
  const analysesMap = useMemo(() => new Map(analyses.map((a) => [a.ticker, a])), [analyses]);
  ...
  const enriched = useMemo(
    () =>
      universe.map((s) => {
        ...
        const normalizedYield = m ? normalizeYieldPercent(m.dividendYield) : null;
  ...
  const filtered = useMemo(() => {
    ...
    if (attrFilter !== "all") {
      list = list.filter((s) => {
        const score = s.attractivenessScore ?? null;
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        ...
        <StatCard motionPreset="surface-enter" label="평균 배당률" value={`${avgYield}%`} sub="metrics 반영 종목 기준" />
        <StatCard motionPreset="surface-enter" label="최장 연속 증가" value={`${maxStreak}년`} sub="유니버스 기준" />
      </div>
      ...
      <SectionCard className="mb-5" motionPreset="surface-enter">
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap items-center gap-2">
              ...
              {(["all", "king", "aristocrat", "growth"] as Category[]).map((c) => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                    category === c ? "bg-indigo-600 text-white" : "text-gray-400 hover:text-white"
                  }`}
                >
              ...
              {([
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                    key === "all" && attrFilter === "all" ? "bg-gray-700 text-white" : cls
                  }`}
                >
                  {label}
                </button>
            ...
            <div className="text-gray-500 text-xs ml-auto self-center">{filtered.length}개 종목</div>
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 text-gray-500 text-xs">
                ...
                <th className="text-right px-4 py-3 cursor-pointer hover:text-white select-none" onClick={() => handleSort("attractiveness")}>
                  <span className="flex items-center justify-end gap-1">매력도 <SortIcon k="attractiveness" /></span>
                </th>
                <th className="text-center px-3 py-3">분석의견</th>
                <th className="text-right px-4 py-3 cursor-pointer hover:text-white select-none" onClick={() => handleSort("streak")}>
                  <span className="flex items-center justify-end gap-1">연속증가 <SortIcon k="streak" /></span>
                </th>
                <th className="text-right px-4 py-3 cursor-pointer hover:text-white select-none" onClick={() => handleSort("price")}>
                  <span className="flex items-center justify-end gap-1">현재가 <SortIcon k="price" /></span>
                </th>
                <th className="text-right px-4 py-3 cursor-pointer hover:text-white select-none" onClick={() => handleSort("dividendYield")}>
                  <span className="flex items-center justify-end gap-1">배당률 <SortIcon k="dividendYield" /></span>
                </th>
                <th className="text-right px-4 py-3">주당배당금</th>
                <th className="text-right px-4 py-3 cursor-pointer hover:text-white select-none" onClick={() => handleSort("peRatio")}>
                  <span className="flex items-center justify-end gap-1">PER <SortIcon k="peRatio" /></span>
                </th>
                <th className="text-right px-4 py-3 cursor-pointer hover:text-white select-none" onClick={() => handleSort("payoutRatio")}>
                  <span className="flex items-center justify-end gap-1">배당성향 <SortIcon k="payoutRatio" /></span>
                </th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((s) => (
                <tr key={s.ticker} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors group cursor-pointer">
                  <td className="px-5 py-3">
                    <Link to={`/watchlist/${s.ticker}`} prefetch="intent" className="flex items-center gap-2.5">
                      <span className="text-base">{s.category === "king" ? "👑" : "🏆"}</span>
                      <div>
                        ...
                        <div className="text-gray-500 text-xs leading-tight max-w-[140px] truncate">{s.name}</div>
                      </div>
                    </Link>
                  </td>

                  <td className="px-5 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${SECTOR_COLORS[s.sector] ?? "bg-gray-800 text-gray-400"}`}>{s.sector}</span>
                  </td>

                  <td className="text-right px-4 py-3">
                    {s.attractiveness ? (
                      <div className="flex flex-col items-end gap-0.5">
                        <span className={`text-sm font-black ${s.attractiveness.color}`}>{s.attractiveness.score.toFixed(1)}</span>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded border ${s.attractiveness.bgColor} ${s.attractiveness.color}`}>{s.attractiveness.label}</span>
                      </div>
                    ) : (
                  ...
                  <td className="text-center px-3 py-3">
                    <span className={`text-[11px] font-semibold px-2 py-1 rounded-md ${getRatingBadge(s.analysis?.overallRating).cls}`}>
                      {getRatingBadge(s.analysis?.overallRating).label}
                    </span>
                  </td>
